...
Blockly.Python['move'] = function(block) {
    ...
    var dropdown_name = block.getFieldValue('NAME');
    // TODO: Assemble python into code variable.
    var code = 'def move(' + number_distance +', pub): rate = rospy.Rate(10); twist = Twist(); speed = ' + dropdown_name +'; twist.linear.z = 0.00; twist.linear.y = 0.15 if speed == "SLOW" else 0.25 if speed == "NORMAL" else 0.75; twist.linear.x = 0.15 if speed == "SLOW" else 0.25 if speed == "NORMAL" else 0.75; movement_time = distance / twist.linear.x; start = time.time(); flag = True; distance_moved = 0.0; print(f"Movendo para frente por {distance} metros..."); while not rospy.is_shutdown() and flag: sample_time = time.time(); elapsed_time = sample_time - start; if elapsed_time > movement_time: flag = False; distance_moved = elapsed_time * twist.linear.x; distance_moved = round(distance_moved, 2); print(f"Movidos: {distance_moved} metros"); pub.publish(twist); twist = Twist(); pub.publish(twist); rate.sleep(); print("Movimento concluído.")';
    return code;
  };
  ...
  Blockly.JavaScript['function_rotate'] = function(block) {
  ...
  Blockly.Blocks['rotate'] = {
    init: function() {
      this.appendDummyInput()
          .appendField("Rotate");
      this.appendDummyInput()
          .appendField("inform the rotation")
          .appendField(new Blockly.FieldDropdown([["LEFT","LEFT"], ["RIGHT","RIGHT"], ["BACK","BACK"]]), "rotate");
      this.setPreviousStatement(true, null);
      this.setNextStatement(true, null);
      this.setColour(230);
   this.setTooltip("");
  ...
  Blockly.Python['rotate'] = function(block) {
    ...
    // TODO: Assemble python into code variable.
    var code = "def rotate("+ dropdown_rotate +", pub): rate = rospy.Rate(10); twist = Twist(); diameter = 0.2; radius = diameter / 2; angular_speed = 0.5; degrees = 90 if direction == 'left' or direction == 'right' else 180 if direction == 'back' else (print('Direção inválida para rotação.') and None); twist.angular.z = angular_speed if direction == 'left' else -angular_speed if direction == 'right' else angular_speed if direction == 'back' else (print('Direção inválida para rotação.') and None); radians = degrees * (3.141592653589793 / 180.0); movement_time = radians / angular_speed; start = time.time(); flag = True; print(f'Girando {direction}...'); while not rospy.is_shutdown() and flag: sample_time = time.time(); elapsed_time = sample_time - start; flag = False if elapsed_time > movement_time else True; degrees_turned = (elapsed_time / movement_time) * degrees; degrees_turned = round(degrees_turned, 2); print(f'Girados: {degrees_turned} graus'); pub.publish(twist); twist = Twist(); pub.publish(twist); rate.sleep(); print(f'Giro {direction} concluído.')\n";
    return code;
  };
